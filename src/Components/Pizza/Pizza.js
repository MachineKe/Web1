...
const Pizza = () => {
  ...
  return (
    <div className="min-h-screen bg-green-100 flex flex-col items-center py-8">
      <h1 className="text-3xl font-bold text-green-800 mb-6">Build Your Own Pizza</h1>
      ...
      <div className="bg-white p-6 rounded-lg shadow-lg w-full sm:w-1/2">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
          {ToppingsData.map(topping => (
            <div key={topping.name} className="flex items-center">
              ...
              />
              <label htmlFor={topping.name} className="ml-3 text-lg text-green-700">
        ...
        <div className="mb-6">
          <p className="text-lg text-green-600">Pizza Base price: £{basePrice}.00</p>
          <h2 className="text-2xl font-semibold text-green-800 mt-4">Ingredients Selected:</h2>
            ...
            {toppings.length > 0 ? (
              toppings.map(topping => (
                <div key={topping.name} className="flex items-center mr-4 mb-4">
                  ...
                  />
                  <span className="text-lg text-green-700">{topping.name}</span>
                </div>
              ))
            ) : (
              <p className="text-lg text-green-500">No toppings selected.</p>
            )}
          </div>
        </div>

        <div className="mb-6">
          <h2 className="text-2xl font-bold text-green-800">Total Price: £{totalPrice.toFixed(2)}</h2>
        </div>
